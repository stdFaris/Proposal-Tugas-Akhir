...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{1.5cm}|p{2.5cm}|X|}
		...
		\hline
		NFR05 & \textit{Reliability} & Jika gambar input tidak dapat diproses dengan akurat, sistem harus menampilkan pesan peringatan atau kesalahan yang informatif \\ 
		\hline
...
\begin{center}
	\begin{tabularx}{\textwidth}{|X|X|X|}
		...
		\begin{enumerate}
			\item Setiap bagian bisa diatur atau diganti secara terpisah untuk hasil terbaik
		...
		\begin{enumerate}
			...
			\item Perlu membuat algoritma tambahan dari nol untuk mendeteksi struktur tabel, terutama untuk tabel yang rumit atau tanpa garis
		\end{enumerate} \\
		\hline
		...
		\begin{enumerate}
			...
			\item Hasil spasi atau pindah baris masih memerlukan perbaikan manual
		\end{enumerate} \\
		\hline
...
\begin{enumerate}
	...
	\par Kriteria ini mengukur kemampuan sistem untuk berkembang di masa depan, seperti menangani volume data yang lebih besar atau kebutuhan kustomisasi baru. Ini adalah pertimbangan jangka panjang yang diberi bobot dua puluh persen.
\end{enumerate}
...
Selanjutnya, pada kriteria \textit{implementability}, perbedaannya menjadi sangat jauh dan menjadi faktor penentu. PaddleOCR menerima skor 5. Alasan utamanya karena PaddleOCR merupakan sebuah solusi terpadu atau \textit{all-in-one}. Sistem ini menyediakan \textit{pipeline} yang sudah terintegrasi penuh, mulai dari deteksi hingga pengenalan tabel, sehingga secara drastis mengurangi waktu dan kompleksitas pengembangan. Sebaliknya, kombinasi YOLO, CRNN, dan \textit{Post-processing} dan kombinasi EasyOCR dan Layout Parser keduanya menerima skor terendah 1. Kedua pendekatan ini mewakili tingkat kompleksitas implementasi yang sangat tinggi, kombinasi YOLO, CRNN, dan \textit{Post-processing} menuntut pembangunan \textit{pipeline} manual dari nol dan kombinasi EasyOCR dan Layout Parser memerlukan usaha integrasi untuk menggabungkan dua sistem terpisah.


Terakhir, pada kriteria skalabilitas yang berfokus pada jangka panjang, situasinya kembali berbalik. kombinasi YOLO, CRNN, dan \textit{Post-processing} dan kombinasi EasyOCR dan Layout Parser keduanya menerima skor 5. Keunggulan utama keduanya terletak pada sifatnya yang modular, yang memungkinkan setiap komponen diganti atau disesuaikan secara independen di masa depan. Di sisi lain, PaddleOCR menerima skor rendah 2. Sifatnya yang sudah terintegrasi erat justru menjadi kelemahan di sini, membuatnya kaku dan sulit untuk dikustomisasi, yang berpotensi menjadi hambatan jika diperlukan modifikasi spesifik di kemudian hari.
